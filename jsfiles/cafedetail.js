...
function initializeNavigation() {
  ...
  if (indexParam && listParam) {
    currentIndex = parseInt(indexParam);
    try {
      listContext = JSON.parse(decodeURIComponent(listParam));
      setupNavigationButtons();
    } catch (error) {
      console.error('list parsing error:', error);
    }
  } else {
    const navElement = document.querySelector('.cafe-navigation');
    if (navElement) {
...
function goBackToCafeList() {
  const params = new URLSearchParams(window.location.search);
  const listParam = params.get('list');

  try {
    if (listParam) {
      const context = JSON.parse(decodeURIComponent(listParam));

      if (context.filters && Object.keys(context.filters).length > 0) {
        const firstFilter = Object.entries(context.filters)[0];
        const [filterType, filterValue] = firstFilter;
        const type = filterType.toLowerCase();
        const value = filterValue;
        const targetUrl = `cafelist.html?type=${type}&value=${encodeURIComponent(value)}`;
...
function setupNavigationButtons() {
  const prevBtn = document.getElementById('prev-cafe');
  const nextBtn = document.getElementById('next-cafe');

  if (!prevBtn || !nextBtn || !listContext) return;

  prevBtn.disabled = currentIndex <= 0;
  prevBtn.addEventListener('click', () => navigateToCafe(currentIndex - 1));

  nextBtn.disabled = currentIndex >= listContext.totalCount - 1;
  nextBtn.addEventListener('click', () => navigateToCafe(currentIndex + 1));
}

function navigateToCafe(newIndex) {
  if (!listContext || newIndex < 0 || newIndex >= listContext.totalCount) {
    return;
  }

  const targetCafe = listContext.cafeIds[newIndex];
  if (!targetCafe) return;

  const filterParams = new URLSearchParams();
  Object.entries(listContext.filters).forEach(([key, value]) => {
    filterParams.set(key, value);
  });

  const newUrl = `cafedetail.html?name=${encodeURIComponent(targetCafe.name)}&id=${targetCafe.id}&index=${newIndex}&list=${encodeURIComponent(JSON.stringify(listContext))}`;
